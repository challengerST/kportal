...
router.get('/editor', function (req, res) {
    var data = req.session.user.member.memberId;
    vipService.info('', data, function (err, body) {
        return res.render('./newVip/vip/editor/view', {
            title: "编辑个人资料"
            , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
            , pos: 'vip'
            , part: 'center'
...
router.get('/reset', function (req, res) {
    return res.render('./newVip/vip/reset/view', {
        title: "修改密码"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'center'
...
router.get('/auth', function (req, res) {
    ...
    vipService.info('', data, function (err, body) {
        ...
        member.level('', req.session.user.company.agentCompanyId, function (err, dt) {
            return res.render('./newVip/vip/auth/view', {
                title: "资质认证"
                , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
                , pos: 'vip'
                , part: 'center'
...
router.get('/relation', function (req, res) {
    return res.render('./newVip/vip/relation/view', {
        title: "收发货人"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'center'
...
router.get('/voucher', function (req, res) {
    return res.render('./newVip/vip/voucher/view', {
        title: "我的抵用券"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'center'
...
router.get('/bill', function (req, res) {
    return res.render('./newVip/bill/list/view', {
        title: "费用清单"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'bill'
...
router.get('/bill/detail', function (req, res) {
    ...
    billService.detail('', id, function (err, body) {
        console.log(body);
        return res.render('./newVip/bill/detail/view', {
            title: "费用清单详情",
            user: 'session' in req && req.session && 'user' in req.session && req.session.user || {},
            pos: 'vip',
            part: 'bill'
...
router.get('/account', function (req, res) {
    return res.render('./newVip/account/list/view', {
        title: "对账单"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'bill'
...
router.get('/account/detail', function (req, res) {
    ...
    account.detail('', id, function (err, body) {
        return res.render('./newVip/account/detail/view', {
            title: "对账单详情",
            user: 'session' in req && req.session && 'user' in req.session && req.session.user || {},
            pos: 'vip',
            part: 'bill'
...
router.get('/appeal', function (req, res) {
    return res.render('./newVip/vip/appeal/view', {
        title: "开票申请"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'bill'
...
router.get('/ticket', function (req, res) {
    return res.render('./newVip/vip/ticket/view', {
        title: "开票资料"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'bill'
...
router.get('/order/detail', function (req, res) {
    ...
    vipService.detail('', data, function (err, body) {
        console.log(body);
        return res.render('./newVip/detail/view', {
            title: "订单详情"
            , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
            , pos: 'detail'
            , data: body
...
router.get('/order/detail/ensure', function (req, res) {
    ...
    vipService.detail('', data, function (err, body) {
        return res.render('./newVip/lading/view', {
            title: "提单确认"
            , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
            , pos: 'detail'
            , data: body
...
router.get('/order/detail/yld', function (req, res) {
    order.declareData('', req.query.id, function (err, body) {
        return res.render('./newVip/yld/view', {
            title: "预录单"
            , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
            , pos: 'detail'
            , data: body
...
router.get('/order/detail/ckd', function (req, res) {
    order.declareData('', req.query.id, function (err, body) {
        return res.render('./newVip/ckd/view', {
            title: "放行通知书"
            , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
            , pos: 'detail'
            , data: body
...
router.get('/note', function (req, res) {
    return res.render('./newVip/note/view', {
        title: "消息中心"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'note'
...
router.get('/noteSet', function (req, res) {
    note.setting('', req.session.user.member.memberId, function (err, body) {
        return res.render('./newVip/noteSet/view', {
            title: "消息设置"
            , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
            , pos: 'vip'
            , part: 'note'
...
router.get('/note/detail', function (req, res) {
    note.detail('', req.query.id || 0, function (err, body) {
        ...
        return res.render('./newVip/noteDetail/view', {
            title: "消息详情"
            , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
            , pos: 'vip'
            , part: 'note'
...
router.get('/entrust', function (req, res) {
    return res.render('./newVip/entrust/list/view', {
        title: "我的委托书"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'entrust'
...
router.get('/entrust/detail', function (req, res) {
    return res.render('./newVip/entrust/detail/view', {
        title: "委托书详情"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'entrust'
...
router.get('/member/list', function (req, res) {
    return res.render('./newVip/member/list/view', {
        title: "员工列表"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'member'
...
router.get('/member/detail', function (req, res) {
    member.detail('', req.query.id, function (err, body) {
        return res.render('./newVip/member/detail/view', {
            title: "员工详情"
            , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
            , pos: 'vip'
            , part: 'member'
...
router.get('/member/role', function (req, res) {
    return res.render('./newVip/member/role/view', {
        title: "角色管理"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'member'
...
router.get('/search/company', function (req, res) {
    return res.render('./newVip/search/company/view', {
        title: "航空公司查询"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'tool'
...
router.get('/search/airports', function (req, res) {
    return res.render('./newVip/search/airports/view', {
        title: "机场查询"
        , user: 'session' in req && req.session && 'user' in req.session && req.session.user || {}
        , pos: 'vip'
        , part: 'tool'
