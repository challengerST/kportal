...
var Listener = (function () {
    ...
    var operateArr = [];
    var listener = function () {
        $(document).on('click', '#pager .pages', function () {
            ...
            if (json && 'resBody' in json && json.resBody && 'sList' in json.resBody && json.resBody.sList && json.resBody.sList instanceof Array && json.resBody.sList.length > 0) {
                ...
                for (var a in arr) {
                    if (arr.hasOwnProperty(a)) {
                        var d = arr[a];
                        if (a == 0) {
                            tel = d.agentMember.mobile;
                            mail = d.agentMember.email;
                        }
                        str += '<option value="' + d.agentMember.memberId + '" data-tel="' + d.agentMember.mobile + '" data-mail="' + d.agentMember.email + '">' + (d.agentMember.name || '/') + '</option>'
                    }
                }
            }
            $('.add-modal').find('.js-mobile').html(tel);
