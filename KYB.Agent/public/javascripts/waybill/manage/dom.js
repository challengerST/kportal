    ...
    var list = function (json, param) {
        ...
        if (json && json.resCode == 0 && 'resBody' in json && json.resBody && 'sList' in json.resBody && json.resBody.sList instanceof Array) {
            if (json.resBody.sList.length > 0) {
                ...
                str = '<tr class="wrong"><td colspan="12">暂无运单号，请添加！</td></tr>';
                $('.list-cout').html('共为您找到 0 条结果');
                $('.page-box').html(' ');
            }
        } else {
            str = '<tr class="wrong"><td colspan="12">请求失败，<a href="">请重试！</a></td></tr>';
            $('.list-cout').html('共为您找到 0 条结果');
            $('.page-box').html(' ');
        }
        function render(d) {
            return '<tr><td>' + (d.airwayBillNum || '/') + '</td> ' +
                ...
                '<a href="javascript:void(0);" style="margin-left: 5px;" class="remove" data-id="' + d.billId + '">删除</a>' +
